...
function ruleEditValidate_isValidPhoneNumber(object) {
    ...
    var nonPhoneNumbersMatch = /[^0-9-x,X\(\)\+\s*]/g;
    var phoneNumberMatch = /\+?\d{1,4}?[-.\s]?\(?\d{1,3}?\)?[-.\s]?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9}/g
    ...
    if (null != illegalChars) {
      ...
      var matchArray = x.match(phoneNumberMatch);
      if((matchArray!== null && (matchArray.length > 1 || matchArray[0] !== x))){
        return display_getValidationError(object, ruleEditValidate_isValidPhoneNumberMsgStr, "", true);
      }
    }
}
...
function isDateTimeDropdownElement(element) {
  if(element.nodeName.toLowerCase()!== 'select'){
    return false;
  }
  var parentElement = element.parentNode;
  if(parentElement && parentElement.hasAttribute("data-calendar")) {
    const hiddenInput = $(parentElement).find('input[type=hidden]')[0];
    if(hiddenInput && hiddenInput.hasAttribute("data-calendar")){
       return true;
       }
    return false;
  }
...
function display_addImageError(error) {
    ...
    if (!offlineError) {
        if(isDateTimeDropdownElement(errorElement)) {
...
function toggleErrorMessage(errorDiv, error, show) {
    ...
    if (show) {
        ...
        for(var i = 1; i < errorDivSpanElements.length; i++ ){
          var currentErrorDivSpan = pega.ctx.dom.getElementsByTagName("SPAN", errorDiv)[i];
          currentErrorDivSpan.title = "";
          currentErrorDivSpan.text = "";
       }
        if(!errorDivFirstSpan.getAttribute("role") && !pega.u.d.displayValidationAlertOverlay) {
          errorDivFirstSpan.setAttribute("role", "alert");
        }
        if (errorDivFirstSpan.className === "inputError") {
            pega.util.Dom.setInnerText(errorDivFirstSpan, errorDivFirstSpan.title);
