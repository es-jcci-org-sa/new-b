pega.ui.RDL = {
  ...
  */
  ...
  getInnerElementsFromRDLRow: function(rdlRow){
    if(!rdlRow) return [];
    // BUG-760537 Consider the Dynamic Layout div having role and tabIndex defined as focusable children of rdlRow
    ...
    //BUG-736372-Removing nested focusable elements such as an image inside a button/link to avoid an extra tab stop
    children = children.filter(child=>!(child.nodeName.toUpperCase() === "I" && child.parentNode && (child.parentNode.nodeName.toUpperCase() === "BUTTON" || child.parentNode.nodeName.toUpperCase() === "A")));
  ...
  addArrowKeyEventListener: function(rdlNode, id){
    if(rdlNode && rdlNode.offsetHeight > 0) {
      ...
      var keyDownHandler = function(ev){
       try{
        ...
        switch(ev.keyCode){
          ...
          //BUG-620926-remove esc key press
      ...
      var keyUpHandler = function(ev){
        ...
        if(ev.keyCode === 27 && ev.target.getAttribute('role')!='row'){
           nodes.forEach(function(node){
              node.setAttribute('tabindex','-1');
            });
            nodes[currentIndex].setAttribute('tabindex','0');
